...
int main() {
    try {
        ...
        map<string, matrix<float,0,1>> known_faces;
        set<string> attendance_log;
        ...
        while (true) {
            ...
            for (auto face : faces) {
                ...
                for (const auto& [name, ref_desc] : known_faces) {
                    float dist = length(ref_desc - face_descriptor);
                ...
                if (label != "Unknown") {
                    if (attendance_log.find(label) == attendance_log.end()) {
                        attendance_log.insert(label);
                        cout << "[" << current_time() << "] " << label << " marked as present." << endl;
                    }
                }

                // Draw box and label
                cv::rectangle(frame, cv::Point(face.left(), face.top()), cv::Point(face.right(), face.bottom()), cv::Scalar(0,255,0), 2);
                cv::putText(frame, label, cv::Point(face.left(), face.top()-10), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255,255,255), 2);
            }
            ...
            if (key == 'a' && !faces.empty()) {
                string name;
                cout << "Enter name for new face: ";
                cin >> name;
                full_object_detection shape = sp(cimg, faces[0]);
                matrix<rgb_pixel> face_chip;
                extract_image_chip(cimg, get_face_chip_details(shape,150,0.25), face_chip);
                known_faces[name] = net(face_chip);
                cout << name << " added to known faces." << endl;
            }
        }
