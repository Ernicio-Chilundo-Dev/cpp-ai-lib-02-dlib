...
// Dlib Face Recognition Network Definetion
    ...
    try{
        ...
        while (true) {
            ...
            for (auto face : faces) {
                ...
                float min_dist = 1.0;
                for (auto& [name, ref_desc] : known_faces) {
                    float dist = length(ref_desc - face_descriptor);
                    if (dist < 0.6 && dist < min_dist) {
                        label = name;
                        min_dist = dist;
                    }
                }
            ...
            cv::imshow("Step 11 - Live Comparison", frame);
